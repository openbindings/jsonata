...
const lanes=['go','javascript','shared','resource','packages','generated','dependency','static'];
...
try{
  ...
  if(lane==='resource'){
    ...
    run('js-resource','javascript','node',['node_modules/mocha/bin/mocha.js','test/evaluation-cancellation.js','test/json-executor.js','test/worker-faults.js','test/worker-protocol.js','test/public-entry.js']);
  }
  if(lane==='packages'){
  ...
  if(lane==='static'){
    ...
    for(const [kind,config] of [['lint','internal/engine/.golangci.yaml'],['security','../maintenance/gosec.yaml']]){
      ...
      if(raw.status!==0)failed=true; // Evidence reviewed != raw clean; CI policy remains explicit.
    }
  }
}catch(error){failed=true;console.error(error.message);}
const report={lane,status:failed?'FAIL_OR_RAW_FINDINGS':'PASS',records,output,publication:false};
