...
// NumericWorkLimits bound decimal computation, not result precision. A budget
// can reject an operation; it cannot select a less accurate successful result.
type NumericWorkLimits struct {
	MaxDigits   uint32
	MaxExponent int32
}
type CompileOptions struct {
	NumericWork *NumericWorkLimits
	// StandardLibraryOnly omits undocumented embedding compatibility helpers.
	// It is not a sandbox for the separate host custom-function APIs.
	StandardLibraryOnly bool
}

// CompileWithOptions snapshots host-owned work budgets for this expression.
// Numeric settings never appear in an expression or its variable bindings.
func CompileWithOptions(expr string, options CompileOptions) (*Expression, error) {
	limits := numeric.DefaultLimits()
	if options.NumericWork != nil {
		limits = numeric.Limits{MaxDigits: options.NumericWork.MaxDigits, MaxExponent: options.NumericWork.MaxExponent}
	}
	...
	return &Expression{
		standardLibraryOnly: options.StandardLibraryOnly,
		numberLimits:        limits,
		src:                 expr,
		ast:                 ast,
...
func NormalizeValue(v any) any {
	...
	case *evaluator.Sequence:
		collapsed := evaluator.ExternalizeLists(val)
...
func (e *Expression) evalCore(ctx context.Context, data any, parent *evaluator.Environment, vars map[string]any) (result any, err error) {
	...
	data = evaluator.InternalizeValue(data)
	env := evaluator.NewChildEnvironment(parent)
	env.ResetCallCounter()
	env.ResetTimestamp()
	env.SetContext(ctx)
	env.SetNumberLimits(e.numberLimits)
	if e.standardLibraryOnly {
		env.UseStandardLibraryOnly()
	}
	env.Bind("$", data)
	for k, v := range vars {
		// A present host variable with a JSON null value is not an absent
		// variable. Language-created undefined bindings still remain undefined.
		if v == nil {
			v = evaluator.Null
		}
		env.Bind(k, evaluator.InternalizeValue(v))
	}
	...
	return evaluator.ExternalizeLists(result), nil
}
...
func (e *Expression) EvalBytes(ctx context.Context, data json.RawMessage) (result any, err error) {
	...
	if needsStringDecode(data, nil) {
		v, decodeErr := evaluator.DecodeJSON(data)
		if decodeErr != nil {
...
func (e *Expression) EvalMap(ctx context.Context, data map[string]json.RawMessage) (result any, err error) {
	...
	if needsStringDecode(nil, data) {
		v, decodeErr := evaluator.DecodeRawMap(data)
		if decodeErr != nil {
			return nil, decodeErr
		}
		return e.Eval(ctx, v)
...
func needsStringDecode(data json.RawMessage, mapData map[string]json.RawMessage) bool {
	if bytes.Contains(data, []byte(`\u`)) {
		return true
	}
	for _, raw := range mapData {
...
func evalComparison(
	...
	case parser.RHSKindNumber:
		...
		match = numeric.Valid(lhs.Raw) && numeric.CompareText(lhs.Raw, c.RHSNumberStr) == 0
...
func gjsonValueToAny(r *gjson.Result) any {
	...
	case gjson.False:
		return false
	case gjson.Number:
		return numeric.FromText(r.Raw)
	case gjson.String:
		return r.String()
	case gjson.JSON:
		if v, err := evaluator.DecodeJSON(json.RawMessage(r.Raw)); err == nil {
			return evaluator.ExternalizeLists(v)
		}
		return nil
	}
	return nil
