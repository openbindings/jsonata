...
func TestEval(t *testing.T) {
	tests := []struct {
		...
		{"power", "$power(2, 8)", nil, float64(256)},
...
func TestRegressionJSON(t *testing.T) { //nolint:funlen // TDT data
	...
	for _, tC := range []struct {
		...
		{
			desc:    "contains_keys_multi_explicit_filter",
			expr:    `$exists($keys(settings)[$contains($, "Endpoint")])`,
...
func TestRegressionExpr(t *testing.T) {
	for _, tC := range []struct {
		...
		// Explicit arrays retain their array kind after deduplication. The old
		// Go-only assertion contradicted the processing model and JS reference.
		desc: "distinct_all_duplicates_literal_preserves_array",
		expr: "$distinct([1, 1, 1])",
		want: []any{float64(1)},
	}, {
