...
func fnFormatNumber(args []any, focus any, env *evaluator.Environment) (any, error) {
	if len(args) < 2 {
	...
	return formatNumberPicture(n, picture, opts, env.NumberLimits())
}
...
func scanSubPictureRegion(runes []rune, fc fmtChars, sp *subPicture) (active []rune, _ error) {
	...
	if start > end {
		return nil, &evaluator.JSONataError{Code: "D3086", Message: "$formatNumber: picture has no digit or separator characters"}
	}
...
func formatNumberPicture(n any, picture string, opts map[string]any, limits numeric.Limits) (string, error) {
	...
	negative := numeric.Compare(n, float64(0)) < 0
	sp := posPic
	if negative {
		sp = negPic
		n, err = limits.Calculate("neg", n, nil, 0)
		if err != nil {
			return "", err
		}
	}
	...
	switch sp.scale {
	case 1:
		n, err = limits.Calculate("*", n, float64(100), 0)
	case 2:
		n, err = limits.Calculate("*", n, float64(1000), 0)
	}
	...
	if sp.expMandatory > 0 {
		result, err = formatWithExponent(n, &sp, fc, limits)
	} else {
		result, err = formatFixed(n, &sp, fc, limits)
	}
...
func formatFixed(n any, sp *subPicture, fc fmtChars, limits numeric.Limits) (string, error) {
	totalFracDigits := sp.fracMandatory + sp.fracOptional
	formatted, err := limits.Fixed(n, totalFracDigits)
	if err != nil {
		return "", err
	}
	...
	if fracStr != "" || sp.hasDecimal {
		return intStr + string(fc.decimalSep) + fracStr, nil
	}
	return intStr, nil
}
...
func formatWithExponent(n any, sp *subPicture, fc fmtChars, limits numeric.Limits) (string, error) {
	N := sp.intMandatory
	...
	mantissa, exp, err := limits.Mantissa(n, N)
	if err != nil {
		return "", err
	}
	mantissaStr, err := limits.Fixed(mantissa, fracSig)
	if err != nil {
		return "", err
	}
	...
	return mantissaPart + string(fc.exponentSep) + expSign + expStr, nil
}
