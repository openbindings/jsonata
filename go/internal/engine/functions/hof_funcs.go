...
// Map/filter/sift supply up to three positional callback arguments. Arity is
// carried by the function value, including native and partially applied values.
func hofArity(fn any) int {
	return min(evaluator.FunctionArity(fn), 3)
}
...
func makeFnReduce(evalFn EvalFn) evaluator.EnvAwareBuiltin {
	return func(args []any, focus any, env *evaluator.Environment) (any, error) {
		...
		if evaluator.FunctionArity(fn) < 2 {
		...
		arrAny := arrayArgument(arrVal)
		reduceArity := min(evaluator.FunctionArity(fn), 4)
