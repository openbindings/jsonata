...
func evalBinary(node *parser.Node, input any, env *Environment) (any, error) { //nolint:gocyclo,funlen // dispatch
	...
	case "+", "-", "*", "/", "%", "**":
		...
		return env.NumberLimits().Calculate(op, left, right, 0)
	...
	case "&":
		...
		return jstring.Concat(ls, rs), nil
...
func evalSubscript(node *parser.Node, input any, env *Environment) (any, error) {
	...
	if IsNumeric(rightVal) {
		i, ok := numeric.Index(rightVal, len(items))
		if !ok {
			return nil, nil
		}
		item := items[i]
...
func filterByPredicate(predicate *parser.Node, items []any, parent any, indexVar string, env *Environment) (any, error) {
	...
	for i, item := range items {
		...
		if val, err := Eval(predicate, item, filterEnv); err != nil {
			return nil, err
		} else if predicateMatches(val, i, len(items)) {
...
func predicateMatches(value any, index, length int) bool {
	if seq, ok := value.(*Sequence); ok {
		value = CollapseSequence(seq)
	}
	if IsNumeric(value) {
		i, ok := numeric.Index(value, length)
		return ok && i == index
	}
	if values, ok := ArrayValues(value); ok {
		allNumbers, match := true, false
		for _, v := range values {
			if !IsNumeric(v) {
				allNumbers = false
				break
			}
			i, ok := numeric.Index(v, length)
			match = match || (ok && i == index)
		}
		if allNumbers {
			return match
		}
...
func selectByIndices(rightVal any, items []any) (any, bool) {
	indexArr, ok := ArrayValues(rightVal)
	if !ok {
		return nil, false
	}
	...
	for _, idxVal := range indexArr {
		...
		if i, ok := numeric.Index(idxVal, len(items)); ok {
...
func evalSubscriptBlockParent(node *parser.Node, input any, env *Environment) (any, error) {
	...
	keepArray := node.KeepArray || hasKeepArrayInChain(node.Left)
	seq := CreateSequence()
	for i, tctx := range tupleCtxs {
		predResult, err := Eval(node.Right, tctx.value, tctx.env)
		if err != nil {
			return nil, err
		}
		if predicateMatches(predResult, i, len(tupleCtxs)) {
	...
	if arr, ok := ArrayValues(result); ok {
		return arr, nil
	}
