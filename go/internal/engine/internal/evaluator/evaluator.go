...
func Eval(node *parser.Node, input any, env *Environment) (any, error) {
	result, err := evalNode(node, input, env)
	if err != nil {
		return nil, err
	}
	return InternalizeValue(result), nil
}

func evalNode(node *parser.Node, input any, env *Environment) (any, error) {
	if node == nil {
		return nil, nil
	...
	case parser.NodeNumber:
		return numeric.FromText(node.Value), nil
