...
func evalPartial(node *parser.Node, input any, env *Environment) (any, error) {
	...
	arity := 0
	for _, placeholder := range isPlaceholder {
		if placeholder {
			arity++
		}
	}
	return NewNativeFunction(partial, arity), nil
}

func callFunction(fn any, args []any, focus any, env *Environment) (result any, err error) {
	defer func() {
		if err == nil {
			result = InternalizeValue(result)
		}
	}()
	if fn == nil {
	...
	for {
		...
		case *RegexValue:
			...
			return applyRegexTest(args[0], f, start, env)
		case *SignedBuiltin:
