...
func evalPathSimple(node *parser.Node, input any, env *Environment) (any, error) {
	...
	for i, step := range node.Steps {
		...
		var err error
		result, err = evalPathStep(step, result, env, prevWasMapper, node.KeepSingletonArray, i == 0)
		if err != nil {
			return nil, err
		}
		...
		if arr, ok := ArrayValues(result); ok && len(arr) == 0 && prevWasMapper {
			if step.Type != parser.NodeName && step.Type != parser.NodeString {
				return nil, nil
			}
		}
		_, isArr := ArrayValues(result)
	...
	for stepIdx, step := range node.Steps {
		...
		if evalStep.Type == parser.NodeBinary && evalStep.Value == "[" &&
			...
			if len(nextCtxs) > 0 {
				...
				if IsNumeric(outerResult) {
					if i, ok := numeric.Index(outerResult, len(nextCtxs)); ok {
		...
		for _, ctx := range ctxs {
			...
			result, err := evalPathStep(evalStep, val, ctx.env, false, node.KeepSingletonArray, stepIdx == 0)
			if err != nil {
				return nil, err
			}
			if result == nil {
...
func buildSortCtxs(left *parser.Node, input any, env *Environment) ([]pathCtx, error) {
	if left.Type != parser.NodePath {
		...
		if rv, ok := ArrayValues(result); ok {
	...
	return expandLastStep(steps[len(steps)-1], prefixCtxs, len(steps) == 1)
}
...
func walkPrefixSteps(steps []*parser.Node, input any, env *Environment, keepSingleton bool) ([]pathCtx, error) {
	...
	for stepIdx, step := range steps {
		...
		for _, ctx := range ctxs {
			...
			result, err := evalPathStep(step, val, ctx.env, false, keepSingleton, stepIdx == 0)
			if err != nil {
				return nil, err
			}
			if result != nil {
...
func expandLastStep(lastStep *parser.Node, prefixCtxs []pathCtx, first bool) ([]pathCtx, error) {
	var ctxs []pathCtx
	for _, ctx := range prefixCtxs {
		...
		result, err := evalPathStep(lastStep, val, ctx.env, false, false, first)
		if err != nil {
			return nil, err
		}
		if result != nil {
...
func appendTupleResults(step *parser.Node, result, parentValue any, parentEnv *Environment, nextCtxs *[]pathCtx) {
	...
	case *Sequence:
		...
		if arr, ok := ArrayValues(collapsed); ok {
...
func expandPathTuple(steps []*parser.Node, ctxs []pathCtx) ([]pathCtx, error) {
	for stepIdx, step := range steps {
		...
		for _, ctx := range ctxs {
			...
			result, err := evalPathStep(step, val, ctx.env, false, false, stepIdx == 0)
			if err != nil {
				return nil, err
			}
			if result == nil {
...
func evalPathStep(
	step *parser.Node, input any, env *Environment, prevWasMapper, keepSingletonArray, first bool,
) (any, error) {
	// Initial * selects the root value's properties. Subsequent .* is a mapper:
	// scalar/function contexts have no properties and contribute no result.
	if first && step.Type == parser.NodeWildcard {
		return Eval(step, input, env)
	}
	// Steps that already handle array inputs natively (field lookup,
	...
	case parser.NodeDescendant:
		...
		seq := CreateSequence()
		if _, isArr := ArrayValues(input); !isArr {
...
func descendantLookup(input any) *Sequence {
	...
	} else if arr, ok := ArrayValues(input); ok {
		for _, item := range arr {
			appendToSequence(seq, item)
			appendToSequence(seq, descendantLookup(item))
		}
