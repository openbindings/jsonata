...
func (l *Lexer) Next(infix bool) (Token, error) { //nolint:gocyclo,funlen // dispatch
	...
	if ch == '/' && !infix {
		...
		patStart, inClass := l.pos, false
		for l.pos < len(l.src) {
			switch c := l.src[l.pos]; c {
			case '\\':
				// An escaped bracket is pattern text, not scanner nesting.
				l.pos++
				if l.pos < len(l.src) {
					l.pos++
				}
			...
			case '/':
				if !inClass {
					...
					if bsCount%2 == 0 {
						...
						flags.WriteByte('g')
						if _, err := ecmaregex.Compile(pattern, flags.String()); err != nil {
							return Token{}, err
						}
...
func (l *Lexer) scanString(quote byte, startPos int) (Token, error) {
	...
	for l.pos < len(l.src) {
		...
		sb.WriteString(jstring.CodeUnit(uint16(r)))
	}
	return Token{}, lexError("S0101", "unterminated string literal")
}
...
func (l *Lexer) scanNumber(startPos int) (Token, error) {
	...
	if !numeric.Valid(numStr) {
		return Token{}, lexError("S0102", "invalid number literal: "+numStr)
	}
	// NumVal is retained only as non-semantic parser metadata. Evaluation and
	// fast paths consume the exact token, including numbers outside binary64.
