...
<p align="center">
  Isolated JSONata qualification candidate — not an adopted or published release.
</p>

See [candidate scope, guarantees and remaining qualification](CANDIDATE.md).
The upstream badges and historical performance examples below do not certify
this modified checkout.
...
- **Bounded regex candidate** — privately instrumented ECMAScript-style matching; see [candidate scope and limits](CANDIDATE.md).
...
| **Array** | `$count` `$append` `$sort` `$reverse` `$shuffle` `$distinct` `$zip` |
| **Object** | `$keys` `$lookup` `$spread` `$merge` `$each` `$sift` `$type` `$error` `$assert` |
...
This isolated candidate is being qualified against the incorporated JSONata
documentation and a separate implementation fidelity contract. Reference
differences must be explained individually; they are not automatically correct.
Do not use numerical tolerances or strip nulls to hide structural or value
differences. See [candidate scope and limits](CANDIDATE.md).

## Regex engine

This candidate no longer uses RE2 for JSONata regex evaluation. See
[candidate scope and limits](CANDIDATE.md) for the engine, work guards,
cooperative cancellation and provenance. It does not claim linear-time matching
or that arbitrary untrusted evaluation is safe without host resource controls.
