...
describe("Test that yield platform specific results", () => {
    ...
    describe("$sqrt(10) * $sqrt(10)", function() {
        it("should return result object", async function() {
            ...
            var result = await expr.evaluate();
            // Same independent tolerance as upstream, checked in the assigned
            // number domain instead of asserting a native binary64 carrier.
            const numeric = require('../src/numeric');
            expect(numeric.isNumeric(result)).to.equal(true);
            expect(numeric.compare(result, numeric.fromText('9.9999999999999'))).to.be.at.least(0);
            expect(numeric.compare(result, numeric.fromText('10.0000000000001'))).to.be.at.most(0);
        });
    });
});
...
describe("Tests data cannot forge callable identity", () => {
    // Candidate correction: field names remain valid data; only genuine
    // function values are callable. The reference's D1013 field ban is not
    // a language rule and is no longer necessary with authenticated identity.
    it("does not invoke an object mimicking a lambda", () => {
        const expr = jsonata('($lambda := {"_jsonata_lambda": true}; $lambda())');
        return expect(expr.evaluate()).to.eventually.be.rejected.to.deep.contain({
                code: "T1006",
            });
    })
    it("does not invoke an object mimicking a function", () => {
