...
describe('Invoke JSONata with callback', function() {
    describe('Make HTTP request', function() {
        it('should return promise to results', async function() {
            // Exercise the real async HTTP callback against deterministic local
            // bytes. Historical public npm availability is not a language gate.
            const server = require('http').createServer((_request, response) => {
                response.setHeader('Content-Type', 'application/json');
                response.end('{"downloads":[{"day":"2016-09-01","downloads":100},{"day":"2016-09-02","downloads":105},{"day":"2016-10-01","downloads":1266},{"day":"2016-11-01","downloads":2783},{"day":"2016-12-01","downloads":2158},{"day":"2017-01-01","downloads":22977},{"day":"2017-02-01","downloads":37728},{"day":"2017-03-01","downloads":46460}]}');
            });
            await new Promise((resolve, reject) => { server.once('error', reject); server.listen(0, '127.0.0.1', resolve); });
            try {
            var expr = jsonata('$httpget($url).downloads{ $substring(day, 0, 7): $sum(downloads) }');
            expr.assign('url', 'http://127.0.0.1:' + server.address().port);
            expr.assign('httpget', httpget);
            await expect(jsonataWithCallback(expr)).to.eventually.deep.equal({
                '2016-09': 205,
                '2016-10': 1266,
                '2016-11': 2783,
                '2016-12': 2158,
