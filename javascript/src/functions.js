...
var unicodeCase = require('./unicode-case');

const functions = (() => {
    'use strict';

    /** Read the active evaluator's immutable numeric work limits.
     * @param {Object} focus - Optional active invocation focus
     * @returns {Object} Numeric work limits
     */
    function numberLimits(focus) {
        return focus && focus.environment ? focus.environment.base.options.numericWork : numeric.defaultLimits;
    }
    ...
    function sum(args) {
        ...
        var total = 0;
        const work = numberLimits(this);
        args.forEach(function (num) {
            total = numeric.calculate('+', total, num, 0, work);
        });
        return total;
    }
    ...
    function max(args) {
        ...
        return args.reduce((a, b) => numeric.compare(a, b) >= 0 ? a : b);
    }
    ...
    function average(args) {
        ...
        return numeric.calculate('/', total, args.length, 0, work);
    }
    ...
    function string(arg, prettify = false) {
        ...
        if (typeof arg === 'string') {
            ...
            str = numeric.stringify(arg, function (key, val) {
    ...
    function substring(str, start, length) {
        ...
        // JSONata documents the substr convention: truncate start and length
        // separately, then clip. Keep exact values until that control boundary.
        start = numeric.clippedTrunc(start, -strLength, strLength);
        if (start < 0) start += strLength;
        var count = typeof length === 'undefined' ? strLength - start :
            numeric.clippedTrunc(length, 0, strLength - start);
        return strArray.slice(start, start + count).join('');
    }
    ...
    function pad(str, width, char) {
        ...
        var result;
        // Retain the candidate Go host's existing bounded output allocation.
        if (numeric.compare(width, -10001) <= 0 || numeric.compare(width, 10001) >= 0) {
            throw {code: 'U_OUTPUT_LIMIT', message: '$pad: width exceeds the host output budget'};
        }
        width = numeric.clippedTrunc(width, -10001, 10001);
    ...
    async function evaluateMatcher(matcher, str) {
        ...
        if(typeof result !== 'undefined' && (!result ||
            ...
            !numeric.isNumeric(result.start) || !numeric.isNumeric(result.end) ||
            !numeric.isInteger(result.start) || !numeric.isInteger(result.end) ||
            numeric.compare(result.start,0)<0 || numeric.compare(result.end,result.start)<0 ||
            (typeof str === 'string' && numeric.compare(result.end,str.length)>0) ||
            !Array.isArray(result.groups) || !isFunction(result.next))) {
    ...
    async function match(str, regex, limit) {
        ...
        if (typeof limit !== 'undefined' && numeric.compare(limit,0) < 0) {
            throw {
                stack: (new Error()).stack,
                value: limit,
                code: 'D3040',
        ...
        if (typeof limit === 'undefined' || numeric.compare(limit,0) > 0) {
            ...
            if (typeof matches !== 'undefined') {
                while (typeof matches !== 'undefined' && (typeof limit === 'undefined' || numeric.compare(count,limit) < 0)) {
                    result.push({
                        match: matches.match,
                        index: matches.start,
    ...
    async function replace(str, pattern, replacement, limit) {
        ...
        if (typeof limit === 'undefined' || numeric.compare(limit,0) > 0) {
            ...
            if (typeof pattern === 'string') {
                ...
                while (index !== -1 && (typeof limit === 'undefined' || numeric.compare(count,limit) < 0)) {
    ...
    function base64encode(str) {
        ...
        // The pinned language defines one byte per character, not UTF-8
        // encoding. The pure codec also rejects non-byte characters on Node,
        // where Buffer's binary encoding otherwise truncates them silently.
        try { return require('core-js-pure/features/btoa')(str); } catch (error) { throw {code:'D3137', message:'$base64encode: characters must be in the byte range'}; }
    }
    ...
    function base64decode(str) {
        ...
        // Decode bytes as UTF-8 as specified, not as Latin-1. Use the host's
        // UTF-8 decoder via percent decoding; malformed text is an explicit
        // failure, never silent replacement or a fabricated surrogate value.
        try {
            const bytes = require('core-js-pure/features/atob')(str);
            const escaped = Array.from(bytes, character => '%' + ('0' + character.charCodeAt(0).toString(16)).slice(-2)).join('');
            return decodeURIComponent(escaped);
        } catch (error) { throw {code:'D3137', message:'$base64decode: invalid base64 or UTF-8 text'}; }
    }
    ...
    async function split(str, separator, limit) {
        ...
        if (typeof limit === 'undefined' || numeric.compare(limit,0) > 0) {
            if (typeof separator === 'string') {
                result = str.split(separator, typeof limit === 'undefined' ? undefined : numeric.clippedTrunc(limit,0,str.length+1));
            } else {
                var count = 0;
                ...
                if (typeof matches !== 'undefined') {
                    ...
                    if (typeof limit === 'undefined' || numeric.compare(count,limit) < 0) {
    ...
    function formatNumber(value, picture, options) {
        ...
        if (pic.picture.indexOf(properties.percent) !== -1) {
            adjustedNumber = numeric.calculate('*', value, 100, 0, work);
        } else if (pic.picture.indexOf(properties['per-mille']) !== -1) {
            adjustedNumber = numeric.calculate('*', value, 1000, 0, work);
        } else {
        ...
        if (pic.minimumExponentSize === 0) {
            ...
            const scaled = numeric.mantissa(adjustedNumber, pic.scalingFactor, work);
            mantissa = scaled.value;
            exponent = scaled.exponent;
        }
        // bullet 6:
        var roundedNumber = numeric.calculate('round', mantissa, undefined, pic.maximumFactionalPartSize, work);
        // bullet 7:
        var makeString = function (value, dp) {
            var str = numeric.fixed(value, dp, work);
            if (zero_digit !== '0') {
    ...
    function formatBase(value, radix) {
        ...
        value = numeric.calculate('round', value, undefined, 0, numberLimits(this));

        if (typeof radix === 'undefined') {
            radix = 10;
        } else {
            radix = numeric.calculate('round', radix, undefined, 0, numberLimits(this));
        }

        if (numeric.compare(radix, 2) < 0 || numeric.compare(radix, 36) > 0) {
            throw {
                code: 'D3100',
                stack: (new Error()).stack,
                value: radix
        ...
        var result = numeric.radixString(value, numeric.clippedTrunc(radix, 2, 36), numberLimits(this));
    ...
    function abs(arg) {
        ...
        result = numeric.calculate('abs', arg, undefined, 0, numberLimits(this));
        return result;
    }
    ...
    function floor(arg) {
        ...
        result = numeric.calculate('floor', arg, undefined, 0, numberLimits(this));
        return result;
    }
    ...
    function ceil(arg) {
        ...
        result = numeric.calculate('ceil', arg, undefined, 0, numberLimits(this));
        return result;
    }
    ...
    function round(arg, precision) {
        ...
        let scale = 0;
        const limits = numberLimits(this);
        if (typeof precision !== 'undefined') {
            if (numeric.compare(precision, -limits.maxExponent) < 0 || numeric.compare(precision, limits.maxExponent) > 0) {
                throw {code: 'U_NUMERIC_LIMIT', message: '$round: scale exceeds the decimal work budget'};
            }
            if (!numeric.isInteger(precision)) {
                throw {code: 'T0410', message: '$round: precision must be an integer'};
            }
            scale = numeric.clippedTrunc(precision, -limits.maxExponent, limits.maxExponent);
        }
        return numeric.calculate('round', arg, undefined, scale, limits);
    }
    ...
    function sqrt(arg) {
        ...
        result = numeric.calculate('sqrt', arg, undefined, 0, numberLimits(this));
    ...
    function power(arg, exp) {
        ...
        if (numeric.compare(arg,0)<0 && !numeric.isInteger(exp) || numeric.compare(arg,0)===0 && numeric.compare(exp,0)<0) {
            throw {code:'D3061', message:'Power has no finite real result'};
        }
        if (numeric.compare(exp,0.5)===0) return numeric.calculate('sqrt',arg,undefined,0,numberLimits(this));
        return numeric.calculate('pow', arg, exp, 0, numberLimits(this));
    }
    ...
    function lookup(input, key) {
        ...
        } else if (input !== null && typeof input === 'object' && !isNumeric(input) && Object.prototype.hasOwnProperty.call(input, key) && !isFunction(input)) {
            result = input[key];
        }
        return result;
    }
    ...
    function spread(arg) {
        ...
        } else if (arg !== null && typeof arg === 'object' && !isLambda(arg) && !isNumeric(arg)) {
    ...
    async function sort(arr, comparator) {
        ...
        if (typeof comparator === 'undefined') {
            ...
            comp = async function (a, b) {
                return isNumeric(a) ? numeric.compare(a, b) > 0 : utils.compareStrings(a, b) > 0;
            };
        } else {
