...
var jsonata = (function() {
    ...
    async function evaluate(expr, input, environment) {
        ...
        if (environment.base.checkAbort) {
            environment.base.checkAbort();
            // A Promise/microtask alone does not let timer/abort events run.
            // Yield in bounded batches, only on signal-bearing evaluations.
            if (++environment.base.checks % 256 === 0 && Date.now() - environment.base.yieldedAt >= 8) {
                await new Promise(resolve => setTimeout(resolve, 0));
                environment.base.yieldedAt = Date.now();
                environment.base.checkAbort();
            }
        }
    ...
    async function evaluateFilter(predicate, input, environment) {
        ...
        if (predicate.type === 'number') {
            var index = numeric.index(predicate.value, input.length);
            ...
            for (index = 0; index < input.length; index++) {
                ...
                if (isArrayOfNumbers(res)) {
                    Array.prototype.forEach.call(res, function (ires) {
                        // round it down
                        var ii = numeric.index(ires, input.length);
                        if (ii === index) {
                            results.push(item);
                        }
                    });
    ...
    async function evaluateBinary(expr, input, environment) {
        ...
        try {
            switch (op) {
                ...
                case '/':
                case '%':
                    result = evaluateNumericExpression(lhs, rhs, op, environment);
                    break;
                case '=':
                case '!=':
        ...
        switch (expr.value) {
            case '-':
                ...
                if(typeof result === 'undefined') {
                    ...
                    result = numeric.calculate('neg', result, undefined, 0, environment.base.options.numericWork);
                } else {
                    throw {
                        code: "D1002",
     ...
     * @returns {*} Result
     */
    function evaluateNumericExpression(lhs, rhs, op, environment) {
        var result;
        ...
        return numeric.calculate(op, lhs, rhs, 0, environment.base.options.numericWork);
    }
    ...
    function evaluateComparisonExpression(lhs, rhs, op) {
        ...
        // type checks
        var ltype = isNumeric(lhs) ? 'number' : typeof lhs;
        var rtype = isNumeric(rhs) ? 'number' : typeof rhs;
        ...
        const order = ltype === 'number' ? numeric.compare(lhs, rhs) : utils.compareStrings(lhs, rhs);
        switch (op) {
            case '<':
                result = order < 0;
                break;
    ...
    function evaluateIncludesExpression(lhs, rhs) {
        ...
        for(var i = 0; i < rhs.length; i++) {
            if(isNumeric(lhs) && isNumeric(rhs[i]) ? numeric.compare(lhs, rhs[i]) === 0 : rhs[i] === lhs) {
                result = true;
                break;
            }
        }
    ...
    function evaluateRangeExpression(lhs, rhs, environment) {
        ...
        if (typeof lhs !== 'undefined' && (!isNumeric(lhs) || !numeric.isInteger(lhs))) {
            throw {
                code: "T2003",
                stack: (new Error()).stack,
                value: lhs
        ...
        if (typeof rhs !== 'undefined' && (!isNumeric(rhs) || !numeric.isInteger(rhs))) {
            throw {
                code: "T2004",
                stack: (new Error()).stack,
                value: rhs
        ...
        const work = environment.base.options.numericWork;
        var exactSize = numeric.calculate('+', numeric.calculate('-', rhs, lhs, 0, work), 1, 0, work);
        var size = numeric.compare(exactSize, 1e7) > 0 ? Infinity : Number(numeric.text(exactSize));
        if(size > 1e7) {
            throw {
                code: "D2014",
                stack: (new Error()).stack,
        ...
        // Native stepping is exact when the entire closed interval is within
        // the safe-integer domain. Wide intervals retain decimal stepping.
        var first = Number(numeric.text(lhs)), last = Number(numeric.text(rhs));
        var safeIntegerRange = Number.isSafeInteger(first) && Number.isSafeInteger(last) &&
            numeric.compare(lhs,first) === 0 && numeric.compare(rhs,last) === 0;
        for (var item = lhs, index = 0; index < size; index++) {
            result[index] = item;
            if (index + 1 < size) item = safeIntegerRange ? first + index + 1 : numeric.calculate('+', item, 1, 0, work);
        }
    ...
    function evaluateRegex(expr, input, environment) {
        ...
        var closure = function(str, fromIndex) {
            var result;
            re.lastIndex = typeof fromIndex === 'undefined' ? 0 : numeric.clippedTrunc(fromIndex, 0, str.length + 1);
            ...
            if(match !== null) {
                ...
                // A match's continuation belongs to that match, not the last
                // unrelated call made through the same regex function.
                var nextIndex = re.lastIndex;
                result.next = function() {
                    if(nextIndex >= str.length) {
                        return undefined;
                    } else {
                        var next = closure(str, nextIndex);
                        if(next && next.match === '') {
    ...
    async function evaluateSortExpression(expr, input, environment) {
        ...
        var comparator = async function(a, b) { 
            ...
            for(var index = 0; comp === 0 && index < expr.terms.length; index++) {
                ...
                // type checks
                var atype = isNumeric(aa) ? 'number' : typeof aa;
                var btype = isNumeric(bb) ? 'number' : typeof bb;
                ...
                var order = atype === 'number' ? numeric.compare(aa,bb) : utils.compareStrings(aa,bb);
                if(order === 0) {
    ...
    function evaluateTransformExpression(expr, input, environment) {
        ...
        var transformer = async function (obj) { // signature <(oa):o>
            ...
            // $clone is a reference embedding compatibility hook, not a
            // documented library function. Closed language embeddings copy
            // internally and do not expose or consult that hook.
            var cloneFunction = environment.base.options.standardLibraryOnly ?
                defineFunction(functionClone, '<(oa)-:o>') : environment.lookup('clone');
    ...
    async function applyInner(proc, args, input, environment) {
        ...
        try {
            ...
            } else if (utils.isNativeFunction(proc)) {
                var focus = {
                    environment: environment,
                    input: input,
                    options: environment.base.options,
    ...
    function evaluateLambda(expr, input, environment) {
        ...
        return utils.functionValue(procedure, 'lambda');
    }
    ...
    function defineFunction(func, signature) {
        ...
        return utils.functionValue(definition, 'native');
    }
    ...
    async function functionEval(expr, focus) {
        ...
        try {
            ...
            if (this.environment.base.checkAbort) {
                this.environment.base.checkAbort();
            }
            populateMessage(err);
            throw {
                stack: (new Error()).stack,
    ...
    function functionClone(arg) {
        ...
        // Cloning is not display formatting: retain each native numeric value.
        // Keep string()'s existing treatment of intermediate function values.
        return numeric.parse(fn.string(arg, false));
    }
     ...
     * @param {boolean} options.standardLibraryOnly: omit undocumented compatibility helpers
     * @returns {{evaluate: evaluate, assign: assign}} Evaluated expression
     */
    function jsonata(expr, options) {
        // Snapshot host settings once; subsequent caller mutation cannot alter
        // a compiled evaluator or an evaluation already in flight.
        options = Object.freeze({...options, numericWork: numeric.normalizeLimits(options && options.numericWork)});
        var ast;
        var errors;
        try {
        ...
        if (options.standardLibraryOnly) environment.bind('clone', undefined);

        environment.bind('now', defineFunction(function(picture, timezone) {
            return datetime.fromMillis.call(this, this.environment.timestamp.getTime(), picture, timezone);
        }, '<s?s?:s>'));
        environment.bind('millis', defineFunction(function() {
            return this.environment.timestamp.getTime();
        }, '<:n>'));
        ...
        return {
            evaluate: async function (input, bindings, callback) {
                ...
                // The third argument may be per-evaluation host options. Keep
                // the upstream callback overload; neither form is a binding.
                const signal = callback && typeof callback === 'object' && callback.signal;
                if (signal) {
                    exec_env.checkAbort = function() {
                        if (signal.aborted) {
                            const aborted = new Error('Evaluation aborted');
